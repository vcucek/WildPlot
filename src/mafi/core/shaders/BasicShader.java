...
public class BasicShader {
	...
	private void printShaderLog(GL gl, int shaderId, String[] files)
	{
		int[] logLength = new int[1];
		gl.glGetShaderiv(shaderId, GL.GL_INFO_LOG_LENGTH, logLength, 0);
		if (logLength[0] > 1) {
	...
	private void printShaderLinkLog(GL gl, int programId, String[] files){
		int[] logLength = new int[1];
		gl.glGetObjectParameterivARB(programId, GL.GL_OBJECT_INFO_LOG_LENGTH_ARB, logLength, 0);
		if (logLength[0] > 1) {
			byte[] log = new byte[logLength[0]];
	...
	protected int loadProgram(GL gl, String[] files)
	{
		printGLLog(gl);
		...
		if (geo) {
			int geometryShaderId = gl.glCreateShader(GL.GL_GEOMETRY_SHADER_EXT);
			...
			gl.glProgramParameteriEXT(programId, GL.GL_GEOMETRY_INPUT_TYPE_EXT, GL.GL_TRIANGLES);
			gl.glProgramParameteriEXT(programId, GL.GL_GEOMETRY_OUTPUT_TYPE_EXT, GL.GL_TRIANGLE_STRIP);
			gl.glProgramParameteriEXT(programId, GL.GL_GEOMETRY_VERTICES_OUT_EXT, 3);
		}
